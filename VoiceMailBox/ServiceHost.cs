    ...
    public class ServiceHost
    {
        public static void Main(string[] args)
        {
            try
            {
                ...
                ActorRuntime.RegisterActorAsync<PrimeNumbersActor>().GetAwaiter().GetResult();

                ServiceEventSource.Current.ActorTypeRegistered(Process.GetCurrentProcess().Id, typeof(VoiceMailBoxActor).ToString());
                ServiceEventSource.Current.ActorTypeRegistered(Process.GetCurrentProcess().Id, typeof(PrimeNumbersActor).ToString());
